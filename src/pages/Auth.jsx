...
export default function Auth({ mode = "signin" }) {
  ...
  const handleSignInSubmit = async (e) => {
    ...
    try {
      const res = await login(signInEmail.trim(), signInPassword, rememberMe);
  ...
  const handleSignUpSubmit = async (e) => {
    ...
    try {
      const res = await register(`${firstName.trim()} ${lastName.trim()}`, signUpEmail.trim(), signUpPassword);
      setToast({
            ...
            <div className="relative bg-bg-card border border-border-dim rounded-[18px] p-[40px_36px]">
              ...
              {tab === "signin" && (
                <div className="animate-fadeIn">
                  ...
                  {/* Tab Row */}
                  <div className="flex bg-bg-input border border-border-dim rounded-[10px] p-1 gap-1 mt-4">
                    <button
